...
**Modes**:
- `copy` — FFmpeg concat demuxer, no re-encode. Lightning fast. **Requires** all inputs to match on a preflight probe check: full stream layout (the `codec_type` sequence in probe order, including subtitle tracks), video `codec_name`/`width`/`height`/`pix_fmt`/`profile`/`sample_aspect_ratio`, audio `codec_name`/`sample_rate`/`channels`/`channel_layout`, and subtitle/data/attachment stream `codec_type`/`codec_name`. Mismatches raise before ffmpeg runs, instead of silently producing a corrupt file (concat demuxer + `-c copy` often exits 0 with glitched output past the first segment, or missing subtitles). `time_base` / `level` / `r_frame_rate` are intentionally **not** compared — the concat demuxer rescales timestamps anyway, and players tolerate level differences, so checking those would over-reject otherwise-safe inputs. The merge itself maps every validated stream (`-map 0`), so extra tracks — e.g. a commentary audio track or subtitle track — survive instead of being silently dropped by ffmpeg's default one-stream-per-type selection.
